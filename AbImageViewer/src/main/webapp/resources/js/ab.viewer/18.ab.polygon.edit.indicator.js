 ...
 * @typedef {Object} AbPolygonEditIndicator.Style
...
/**
 * 다각형 도형 편집 지시자 스타일 정보
 * @typedef {Object} AbPolygonEditIndicator.Controls
 ...
 * @param {AbPolygonEditIndicator.Style} [options.controls.style] 스타일 설정
 */
function AbPolygonEditIndicator(options){
	if (!options) options = {};
	...
	/**
	 * 편집점 사용 여부
	 * @type {AbPolygonEditIndicator.Controls}
	 */
	this.controls = {
		/**
		 * 회전 편집점 사용 여부
	...
	/**
	 * 스타일 정보
	 * @type {AbPolygonEditIndicator.Style}
	 */
	this.style = {
		/**
		 * 기본 스타일
	...
	/**
	 * 다각형 편집점 업데이터 인스턴스
	 * @type {AbShapePolygonPointUpdater}
	 */
	this.updater = new AbShapePolygonPointUpdater();
};
...
AbPolygonEditIndicator.prototype = {
	constructor: AbPolygonEditIndicator,
	...
	editable: function (px, py, ctx){
		...
		var len = this.target.points.length;
		for (var i=0; i < len; i++){
			var p = this.target.points[i];

			pc.x = p.x * page.scale.x + x;
			pc.y = p.y * page.scale.y + y;

			//console.log('[CONTAINS]['+i+'] pc.x=' + pc.x + ', pc.y=' + pc.y + ', px=' + px + ', py=' + py);

			if (this.containsControl(pc.x, pc.y, px, py)){
	...
	editPos: function (point){
		...
		if (AbGraphics.isPolygonEditPoint(point)){
			var n = AbGraphics.getPolygonEditPointIndex(point);
			var p = this.target.points[n];

			return { x: p.x * page.scale.x + tbox.x, y: p.y * page.scale.y + tbox.y };
		}
	...
	resize: function (point, px, py){
		...
		if (point == 'A'){
			...
			return incAngle;
		}else if (AbGraphics.isPolygonEditPoint(point)){
			var n = AbGraphics.getPolygonEditPointIndex(point);
			...
			pc = page.fromCanvas(px, py);

			this.updater.set(this.target);
			this.updater.begin(n, pc.x, pc.y);

			//console.log('[CHECK] PX=' + px + ', PY=' + py);
			//console.log('\t[BEFORE]['+this.updater.data.before.edit.full+'] ', this.updater.data.before);
			//console.log('\t[AFTER]['+this.updater.data.after.edit.full+'] ', this.updater.data.after);

			var point = this.updater.data.after.edit.full;

			if (this.updater.changed()){
				point = this.updater.data.before.edit.full;

				var b = this.updater.data.after.boundary;
				var amin = page.toCanvas(b.x, b.y);
				var amax = page.toCanvas(b.x + b.width, b.y + b.height);
				...
				//console.log('\t[MODIFIED] POINT='+point+', PX=' + px + ', PY=' + py);
			}
			...
			if (point != 'CC'){
				...
				this.target.move(box.x, box.y);

				//console.log('[MOVE] x=' + box.x + ', y=' + box.y);
			}else{
				this.updater.end();
			}
	...
	draw: function(ctx){
		...
		for (var i=0; i < len; i++){
			...
			this.drawControl(ctx, Math.round(p.x * scaleX), Math.round(p.y * scaleY));
		}
