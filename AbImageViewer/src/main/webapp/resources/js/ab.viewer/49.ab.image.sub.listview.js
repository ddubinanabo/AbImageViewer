 ...
 * @param {String} [options.templateSelector=#sub-list-template] 아이템 템플릿 선택자
 ...
 */
function AbImageSubListView(options){
	if (!options) options = {};
	 ...
	 * @type {String}
	 */
	this.selector = options.selector || '#sub-thumbnails';
	 ...
	 * @type {String}
	 */
	this.templateSelector = options.templateSelector || '#sub-list-template';
	/**
...
AbImageSubListView.prototype = {
	constructor: AbImageSubListView,
	...
	notifyObservers: function(topic, value, element){
		this.exec(function(){
			...
			for (var i=0; i < len; i++){
				...
				else if (typeof o.subListViewNotify == 'function')
					o.subListViewNotify(this, topic, value, element);
			}
		});
	},
	...
	notify: function(sender, topic, value){
		...
		switch (topic){
		case 'sub.page.select':
			if (this.enabled()){
				this.select(value);
			}
			break;
		case 'sub.collection.changed':
			if (this.enabled()){
			...
			break;
		case 'sub.modified':
			this.updateItemProperty(value.uid, {
				image: value.data,
			});
	...
	notifyAbsolute: function(sender, topic, value){
		//console.log('[SUB][LISTVIEW::'+this.name+'][Viewer]['+ topic + '] ' + value);
		...
		case 'sub.page.add':
			...
			break;
		case 'sub.page.loading':
			this.map[value].attr('lt-status', 'loading');
			break;
		case 'sub.page.loaded':
			uid = value.page.uid;
			index = value.index;
		...
		case 'sub.page.remove':
			this.removeList();
			break;

		case 'sub.page.remove.list':
			// 삭제된 페이지의 뷰 적용
			var a = value.pages;
	...
	renderList: function (options){
		...
		for (var i=start, r = 0; i < end; i++, r++){
			...
			var uid = e.attr('lt-uid');
			var page = this.pages.getById(uid);
	...
	listItemImage: function (value){
		...
		var ae = this.topic(item, 'annotation');
	 ...
	 * @private
	 * @param {(String|Number)} value 문자열이면 {@link AbPage|페이지} UUID, 숫자형이면 아이템 인덱스입니다.
	 * @param {Object} [options] 프로퍼티 정보
	 * @param {String} [options.image] 이미지 데이터
	 */
	updateItemProperty: function(value, options){
		if (!options) options = {};
		...
		if (page){
			if (options.hasOwnProperty('image'))
				this.topic(item, 'image').attr('src', options.image);
